...
import { Card, Col, Container, Image, Pagination, Row, Table } from "react-bootstrap";
...
const Mybooking = () => {
  ...
  useEffect(() => {
    axios
      ...
      .then((res) => {
        setBooking(res.data.sort((a, b) => b.bookingId - a.bookingId));
      })
      .catch((err) => {
        console.log(err);
      });
          ...
          <Card.Body data-aos="fade-left">
            ...
            {currentBookings &&
              currentBookings.map((book, index) => (
                <Card className="my-3" data-aos="fade-left">
